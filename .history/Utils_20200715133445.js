...
async function getOurRecipeInfo(recipeID) {
  ...
  return { ...recipe, ingredients: [...recipeIngridients], instructions: [...recipeInstructions] };
}
...
async function checkIfUserWatchedAndFavorited(userID, recipe) {
  recipe.watched = await checkIfIsBeenWatched(userID, recipe.id, 1);
  recipe.liked = await checkIfIsFavorite(userID, recipe.id, 1);
  return recipe;
}
