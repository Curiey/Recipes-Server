...
async function addinstructionToRecipe(recipeID, instruction) {
  let { id, text } = instruction;
  await DButils.execQuery(`INSERT INTO RecipeIngredients(recipeID, instructionID, context) VALUES ('${recipeID}', '${id}', '${text}') SELECT SCOPE_IDENTITY() AS id`)
    .catch((error) => next(error));
}
 ...
 */
function addinstructions(recipe, instructions) {
  if (recipe == undefined || instructions == undefined) new Error("got bad argument. recipeID or instructions cannot be found.")
  let { id } = recipe[0];
  instructions.map((instruction) => addinstructionToRecipe(id, instruction));
}
