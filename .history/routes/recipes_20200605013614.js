...
async function checkIfIsBeenWatched(userID, recipeID) {
  let answer = await DButils.execQuery("SELECT * FROM Watched WHERE userID='" + userID + "' and recpeID='" + recipeID + "'");
  if(answer.length == 0) {
    return false;
  }
  else {
  ...
  async function checkIfIsFavorite(userID, recipeID) {
    let answer = await DButils.execQuery("SELECT * FROM Favorites WHERE userID='" + userID + "' and recpeID='" + recipeID + "'");
    if(answer.length == 0) {
      return false;
    }
    else {
