...
function async getInstruction(instruction) {
  let { number, step, ingredients } = instruction;
  let ingredientsAfterParse = await Promise.all(
...
async function extractDataFromRecipe(recipeDataSpooncular)
{
  let { id, serving, image, title, readyInMinutes, vegan, vegetarian, aggregateLikes } = recipeDataSpooncular;
  ...
  return { id, serving, image, title, readyInMinutes, vegan, vegetarian, aggregateLikes, instructions, ingredients };
